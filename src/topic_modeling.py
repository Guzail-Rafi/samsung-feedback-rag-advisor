...
def main():
    ...
    # Use useful English comments only
    df_main = df[
    ...
    if all_comment_outputs:
        final_comments = pd.concat(all_comment_outputs, ignore_index=True)
    else:
        final_comments = pd.DataFrame()

    if all_topic_outputs:
        final_topics = pd.concat(all_topic_outputs, ignore_index=True)
    else:
